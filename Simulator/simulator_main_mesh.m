...
%init for FFT
global FFTTraffic;
FFTTraffic = fft_traffic_dist(MaxCoreNumber);
global DestinationArbitStatus;
DestinationArbitStatus= zeros(1,MaxCoreNumber);
global HotSpotOrUniform;
HotSpotOrUniform=ones(1,MaxCoreNumber);
%naseef mansoor
...
TileDimension = [8 8]; % tile dimension in a mesh default [8 8]
global TileSize;
TileSize=[20 20];
global XDim; % 1
XDim = 1;
...
InjectionMatrix = xlsread('injectionrates.xlsx','FFT Normal');
% Alloc_Map = xlsread('Results_All.xlsx','FFT - SW','f3:f66');
% Alloc_Map = Alloc_Map';
% % ---------------------------------------------
% % load TaskAllocation.mat;
% InjectionMatrix = zeros(64,64);
% InjectionRate = xlsread('injectionrates.xlsx','FFT Normal');
% for i = 1:64
%     for j = 1:64
%         InjectionMatrix(i,j)=InjectionRate(Alloc_Map(i),Alloc_Map(j));
%     end
% end
% % ---------------------------------------------

% Normalize Injection Matrix
% InjectionMatrix = InjectionMatrix/8;
% MaxInj = max(max(InjectionMatrix));
% InjectionMatrix = InjectionMatrix.*(1/MaxInj);
% InjectionMatrix = InjectionMatrix / MaxSwitchNumber;
...
%Connectivity( :, :,1) = xlsread('256coreconnectivity.xls');
...
global SwitchFlit;
SwitchFlit = zeros(1,SwitchTotal);
...
PortNumber(1:MaxSwitchNumber) = PortNumberTemp;
...
for iMsgNo= 1:size(MsgStatus,1)
    if((MsgEnergyStatus(iMsgNo,MsgLength,FlitAbsorbCycle)~=0)&&(MsgEnergyStatus(iMsgNo,1,FlitInjectCycle)>ResetCycle))
        ...
        while(DestinationIndex ~=MsgStatus(iMsgNo,Destination))
            % Naseef Mansoor
            disp(['Message from ' num2str(MsgStatus(iMsgNo,Destination)) 'to' num2str(MsgStatus(iMsgNo,Source))]);
...
%              ManhattanDistance=abs(SourceX-DestinationX)+abs(SourceY-DestinationY);
            ...
            disp(['Message No. - ' num2str(iMsgNo) ' Link from ' num2str(SourceIndex) ' to ' num2str(DestinationIndex)]);
            if(foundWi1 == 1 && foundWi2 == 1)
				...
				%LinkEn(SourceIndex,DestinationIndex) = LinkEn(SourceIndex,DestinationIndex) + (2.3*32)*MsgLength;
            ...
            %  0.4 pj per mm * 32 bits
				...
				LinkEn(SourceIndex,DestinationIndex) = LinkEn(SourceIndex,DestinationIndex) + (0.2*LinkData(SourceIndex,DestinationIndex,1)*32)*MsgLength;
...
Latency
% MsgRoutingScheme
LinkEn=LinkEn*(2.5/1000)/3500;
SwitchFlit = SwitchFlit*(2.5/1000)/3500;
for i = 1:64
    for j = i:64
        LinkEn(i,j) = LinkEn(i,j) + LinkEn(j,i);
        LinkEn(j,i) = LinkEn(i,j);
    end
end
save('FFTThermal.mat','LinkEn','SwitchFlit','TotalEnergyPerMessage','TotalEnergyPerMessageForTokenRecoveryScheme','avghop','Input','Throughput','ThroughputAfterBlockingToken','Latency');
