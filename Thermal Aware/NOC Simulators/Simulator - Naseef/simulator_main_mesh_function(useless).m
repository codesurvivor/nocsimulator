function [ SimulationResult ] = simulator_main_mesh_function( )
...
EnabledWirelessDataTransmission=0;
...
TotalEnergy=0;
ValidMessageCount=0;
for MsgNo= 1:size(MsgStatus,1)

    if((MsgEnergyStatus(MsgNo,MsgLength,FlitAbsorbCycle)~=0)&&(MsgEnergyStatus(MsgNo,1,FlitInjectCycle)>ResetCycle))
        ...
        while(DestinationIndex ~=MsgStatus(MsgNo,Destination))
            ...
            %  0.4 pj per mm * 32 bits
                MessageLinkEnergy = MessageLinkEnergy + (0.4*32);
            end
        ...
        while (FlitNo <= MsgLength)
            ...
            FlitCycles=MsgEnergyStatus(MsgNo,FlitNo,FlitAbsorbCycle)-MsgEnergyStatus(MsgNo,FlitNo,FlitInjectCycle);

            MessageEnergy=MessageEnergy + MessageLinkEnergy + MsgStatus(MsgNo,SwitchCycles)*200 +  MsgStatus(MsgNo,BufferCycles)*80;
            FlitNo=FlitNo+1;
        end

        TotalEnergy=TotalEnergy+MessageEnergy;
...
% SimulationResult a 1D matrix
% 1=Throughput
% 2=Throughput after blocking
% 3=Latency
% 4=Hop Count 

SimulationResult=[Throughput ThroughputAfterBlockingToken Latency avghop];
% MsgRoutingScheme
